...
>
    ...
    <table class="leave-table">
        <tr style="min-height: 12mm;">
            <td width="25%">
                ...
                <input type="date" wire:model="data.date_of_filing" class="leave-input" readonly style="margin-top:2px;" />
            </td>
            <td width="37.5%">
    ...
    <table class="leave-table">
        <tr style="min-height: 88mm; vertical-align: top;">
            ...
            <td width="50%">
                ...
                {{--
                    :class uses the Alpine `leaveType` data property (plain JS, no $wire).
                    @click calls selectLeaveType() — defined in <script>, no $ in the attribute.
                --}}
                ...
                @if(($this->data['type_of_leave'] ?? '') === 'others')
            ...
            <td width="50%">
                ...
                <div class="leave-detail-section">
                    ...
                    <div style="display:flex; align-items:center; gap:3px; margin:3px 0; flex-wrap:wrap;">
                        ...
                        ></span>
                        Abroad (Specify)
                        @if(($this->data['vacation_location'] ?? '') === 'abroad')
                ...
                <div class="leave-detail-section">
                    ...
                    <div style="display:flex; align-items:center; gap:3px; margin:3px 0; flex-wrap:wrap;">
                        ...
                        @if(($this->data['sick_leave_location'] ?? '') === 'in_hospital')
                    ...
                    <div style="display:flex; align-items:center; gap:3px; margin:3px 0; flex-wrap:wrap;">
                        ...
                        @if(($this->data['sick_leave_location'] ?? '') === 'out_patient')
                ...
                {{-- Special Leave Benefits for Women (free text only) --}}
                <div class="leave-detail-section">
    ...
    <table class="leave-table">
        <tr style="min-height: 22mm;">
            <td width="50%">
                ...
                {{-- Working days display — updated by Alpine directly --}}
                <div style="margin-top: 3px;">
                    <input
                        type="number"
                        class="leave-input"
                ...
                <div class="leave-grid-2" style="margin-top: 2px;">
                    <div>
                        ...
                        {{--
                            No wire:model here — Alpine manages value directly via x-ref.
                            @change calls onFromChange() which handles Livewire sync + working days.
                        --}}
                        <input
                            type="date"
                            class="leave-input"
    ...
    @if(($this->data['type_of_leave'] ?? '') === 'sick_leave' && ($this->data['number_of_working_days'] ?? 0) >= 3)
...
{{-- ================================================================
     ...
     HTML attributes only call named methods like selectLeaveType(),
     toggleField(), onFromChange() — zero $ signs in any attribute.
================================================================ --}}
<script>
function leaveForm() {
    return {

        // ── Alpine-local state (mirrors Livewire data) ────────────────
        // Keeping local copies means :class bindings are instant —
        // no async $wire.get() calls needed in attributes.
        leaveType:          null,
        ...
        // Date limit strings bound to :min/:max
        today:              '',
        vacationMin:        '',
        vacationMinDisplay: '',
        fromMin: '',
         ...
         * Returns null if either date is missing or from > to.
         */
        ...
        // ── Checkbox toggle methods (called from @click) ──────────────

        /**
         * Generic single-select toggle for any field group.
         * localProp  — the Alpine data property name (string key)
         * value      — the value to toggle
         * wirePath   — the Livewire data path e.g. 'data.vacation_location'
         */
        ...
        /**
         * Leave type toggle — also clears dates and refreshes limits.
         */
        async selectLeaveType(key) {
            ...
            // Clear dates when leave type changes
            this.dateFrom    = '';
            this.dateTo      = '';
        ...
        async init() {
            ...
            // Read existing Livewire state (handles edit mode / page reload)
            this.leaveType         = await this.$wire.get('data.type_of_leave')         || null;
            this.vacationLocation  = await this.$wire.get('data.vacation_location')      || null;
            this.sickLeaveLocation = await this.$wire.get('data.sick_leave_location')    || null;
            this.studyLeavePurpose = await this.$wire.get('data.study_leave_purpose')    || null;
            this.otherPurpose      = await this.$wire.get('data.other_purpose')          || null;
            this.commutation       = await this.$wire.get('data.commutation')            || null;
            this.dateFrom          = await this.$wire.get('data.leave_date_from')        || '';
            this.dateTo            = await this.$wire.get('data.leave_date_to')          || '';
            this.workingDays       = await this.$wire.get('data.number_of_working_days') || null;

            this.updateDateLimits();

            // Keep local state in sync if the hidden Filament section fields are used
